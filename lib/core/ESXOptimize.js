...
import { compare, hasStyleScoped, createStaticReferenceNode, getMethodAnnotations, toFirstUpperCase,toCamelCase,hasDynamicRef} from "./Common";
 ...
 * 6、ELEMENT_CLASS 和 ELEMENT_STYLE 在元素上需要应用标志位，组件上可以排除
 */
...
const enableBlockNodes = ['if','elseif','else','for','each']
...
function isArrayVNode(type, flag=false){
    ...
    if(type.isTupleType && Utils.isType(type)){
    ...
    if(flag){
        ...
        if(origin && Utils.isModule(origin)){
            if(Namespace.globals.get('VNode').is(origin)){
                return true;
            }
        }
    }
...
function createDirectiveDescriptor(ctx, stack, child, prev=null){
    ...
    if(stack.isJSXExpressionContainer){
        if(isArrayVNode(stack.type())){
            child = createFragmentVNode(
                ctx,
                child,
                ctx.createObjectExpression([createElementKeyNode(ctx, stack)]),
                null
            )
        }else if(!child.isExplicitVNode){
            child = createNormalVNode(ctx, child, false, false, stack)
        }
        const scope = stack.scope.forContextScope;
        if(scope && hasDynamicRef(scope, stack.expression)){
            child.hasDynamicRefs = true;
        }
    }
    ...
    while( directives.length > 0){
        ...
        if( name ==="each" || name ==="for" ){
            ...
            child = createFragmentVNode(ctx, child, ctx.createObjectExpression([createElementKeyNode(ctx, stack)]), hasKeyAttribute ? ELEMENT_KEYED_FRAGMENT : ELEMENT_UNKEYED_FRAGMENT, true);
...
function createChildren(ctx, children, data){
    ...
    let next = ()=>{
        if(index<len){
            ...
            if(descriptor.child.hasDynamicRefs){
                hasDynamicRefs = true;
        ...
        if(last){
            ...
            if( hasIf ){
                ...
                if( result && result.cmd.includes('elseif') ){
                    ...
                    const endNode = last.stack.isSlot && !last.stack.isSlotDeclared ? ctx.createLiteral(undefined) : createCommentVNode(ctx, 'end if', true)
    ...
    if(hasConditionDirective){
        data.hasConditionDirective = true;
    }

    if(!pureStaticChild){
        data.pureStaticChild = false;
    }

    if(hasDynamicRefs){
        data.hasDynamicRefs = true;
    }
...
function createAttributes(ctx, stack, data){
    ...
    stack.openingElement.attributes.forEach(item=>{
        ...
        // if(propValue && (propValue.type==="ObjectExpression" || propValue.type==="ArrayExpression" || propValue.type==="FunctionExpression" || propValue.type==="ArrowFunctionExpression")){
        //     propValue = needAddForIndexOfCacheNode(ctx, stack, propValue)
        // }
    ...
    if(!pureStaticAttributes){
        data.pureStaticAttributes = false;
    }
    if(hasDynamicRefs){
        data.hasDynamicRefs = hasDynamicRefs
    }
}
...
const excludes = ["patchFlag", "hasMultipleChildNodes","pureStaticAttributes","pureStaticChild","hasConditionDirective","hasKeyReference"];
...
function createDirectiveElementNode(ctx, stack, children, hasKeys, hasDynamicChild=false){
        ...
        case 'each' :
            {
                ...
                return createFragmentVNode(ctx, node, ctx.createObjectExpression([createElementKeyNode(ctx, stack)]), hasKeys ? ELEMENT_KEYED_FRAGMENT : ELEMENT_UNKEYED_FRAGMENT, false);
            }
    } 
    return null;
}
...
function getElementStats(ctx, stack, data, children, isRoot=false){
    ...
    let hasDynamicRefs = !!data.hasDynamicRefs;
    ...
    isForVNode = stack.directives ? stack.directives.some( (b)=>{
        const bb = b.name.value().toLowerCase();
        return bb === 'each' || bb ==="for";
    }) : false;
...
function createElement(ctx, stack){
    ...
    if(componentDirective){
        ...
        if(children.length>1){
            ...
            if(isRoot || pIsRoot){
                const parent = stack.getParentStack(parent=>parent.isFunctionExpression);
            ...
            if(stack.isJSXFragment || (isRoot && stack.openingElement.name.value()==='root')){
                ...
                if(isWebComponent && !isTeleport){
                    ...
                    if( properties.length > 0 ){
                        const flags = (data.patchFlag & ELEMENT_DYNAMIC_SLOTS) ? 2 : 1
                        properties.push( ctx.createProperty(
                ...
                if(!isBlock && stack.jsxRootElement === stack.parentStack && stack.parentStack.openingElement.name.value()==='root'){
                    const rawChildren = getChildren(stack.parentStack);
        ...
        if(!isForVNode){
            nodeElement.hasDynamicRefs = hasDynamicRefs;
        }
