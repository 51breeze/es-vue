...
function Component(props){
    if( !hasOwn.call(this, privateKey) ){
        ...
        const propsData = props && typeof props === 'object' ? Object.create(props) : Object.create(null);
...
function injectFactor(name, from, defaultValue){
    ...
    if( value !== void 0 ){
        const setValue = ()=>{
            const _value = this.receivePropValue(value, name);
            if(descriptor.isAccessor()){
                if(descriptor.setter){
                    descriptor.invokeSetter(this,_value);
                }else{
                    throw new Error(`Injector property '${name}' is readonly.`);
                }
            }else if(descriptor.isMethod()){
                descriptor.invokeMethod(this, _value);
            }else if(descriptor.isProperty()){
                descriptor.setPropertyValue(_value)
            }
        }
        if(Vue.isRef(value)){
            Vue.watch(value, setValue)
        }
        setValue();
    }
    return value;
...
Object.defineProperty( proto, 'reactive', {value:function reactive(name, value, initValue){
    ...
    const stateRefs = this[privateKey].stateRefs;
...
Object.defineProperty( proto, 'getCacheForVNode', {value:function getCacheForVNode(key){
    const target = this[privateKey];
    if(!target.renderCached){
        target.renderCached  = [];
    }
    let cached = target.renderCached;
    if(key===true){
        cached[privateKey] = null;
    }else{
        let dataset = cached[privateKey] || (cached[privateKey] = Object.create(null));
        cached = dataset[key] || (dataset[key]=[]);
        cached.index = 0;
    }
    return cached;
}});
...
Object.defineProperty( proto, 'watch', {value:function watch(name, callback, options){
    if(typeof name !=='string'){
        return Vue.watch(name, callback, options);
    }
    const target = this[privateKey].instance;
    if(target){
        const segs = String(name).split('.').map(val=>val.trim());
        let first = segs.shift();
        if(first==="this"){
            first = segs.shift();
        }
        const descriptor = Reflect.getDescriptor(this,first);
        options = options === true ? {immediate:true} : options;
    ...
    if(classDescriptor){
        ...
        while(inheritClass && (inheritClass = inheritClass.inherit) && inheritClass !== Component ){
            if('__vccOpts' in inheritClass){
                ...
                if(!options.__scopeId && vccOpts.__scopeId){
                    options.__scopeId = vccOpts.__scopeId;
                }
        ...
        if( options.props ){
            ...
            for(let key in propsData){
                ...
                if( esInstance.beforeReceiveProp(propsData[key],key) ){
                    ...
                    if( descriptor && descriptor.isMemberDescriptor){
                        if(descriptor.isAccessor() && descriptor.isPublic()){
                            const setter = descriptor.setter;
                            if( setter ){
...
function _normalVNode(value, optional={}){
    if( Array.isArray(value) ){
        if(optional.optimize){
            Vue.openBlock()
            return Vue.createElementBlock(
                Vue.Fragment, 
                {key:optional.key}, 
                value.map(child=>_normalVNode(child, optional))
            );
        }
...
Object.defineProperty( Component, 'normalVNode', {value:function normalVNode(value, optional={}){
    return _normalVNode(value, optional)
}});
